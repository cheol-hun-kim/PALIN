...
def _generate_local_knowledge_reply(message: str, history: list = None, user_role: str = "STUDENT", school_level: str = "HIGH") -> str:
    ...
    if any(k in msg_clean for k in ["2달", "1달", "3달", "100일", "50일", "30일", "d-", "파이널", "막판", "남은 시점", "남은시점", "남은 기간", "얼마 안", "마무리", "시점에", "지금 시점"]):
        return (
            ...
            "4. 3단계 오답노트 백지 재풀이: 틀린 문제를 3일 뒤 빈 시험지에 100% 스스로 손으로 풀어낼 수 있는지 검증할 것.\n\n"
    ...
    if any(k in msg_clean for k in ["계획", "시간표", "순공", "타이머", "168", "집중", "자습", "공부 시간", "루틴"]):
        return (
            ...
            "1. 30분 단위 블록 계획표: 하루를 30분 단위로 쪼개어 성취감을 시각화하고 미루는 습관을 차단해.\n"
            "2. PASS-MATE 집중 타이머: 폰을 멀리 두고 스톱워치로 순수 자습 4시간을 매일 정밀 측정해.\n"
            "3. 주간 피드백: 매주 일요일 밤 168시간 중 낭비된 구멍(유튜브, 침대 뒹굴기)을 색출하고 다음 주 계획에 메워 넣어야 성적이 수직 상승한다.\n\n"
    ...
    if any(k in msg_clean for k in ["수학", "탐구", "과탐", "사탐", "영어", "킬러", "준킬러", "개념", "공부법"]):
        return (
            ...
            "1. 개념의 백지 증명: 공식을 눈으로 외우지 말고 빈 종이에 유도 과정과 성질을 직접 손으로 증명해봐.\n"
            "2. 조건 해석 훈련: 문제 발문의 모든 조건에 밑줄을 긋고, '출제자가 이 조건을 왜 주었는가?'를 연결하는 발상을 기록해.\n"
            "3. 3단계 오답노트: 틀린 문제는 해설지를 덮고 3일 뒤 백지 상태에서 100% 풀어내야 진짜 내 실력이 된다.\n\n"
    ...
    if any(k in msg_clean for k in ["불안", "슬럼프", "포기", "막막", "멘탈", "자신감", "우울", "힘들어", "스트레스", "망칠까봐"]):
        return (
            ...
            "남과의 비교, 시험 결과에 대한 막연한 공포는 당장 내려놓자. 수능은 남을 이기는 시험이 아니라, 시험장에서 내가 아는 문제를 실수 없이 완벽히 건져 올리는 '내면의 싸움'이야.\n\n"
    ...
    return (
        ...
        "지금 가장 점수가 안 나오거나 막막한 과목(국어/수학/탐구)이나 현재 등급을 구체적으로 알려줘. 바로 1:1 역전 플랜을 짜줄게!"
    )
