...
def get_exam_source_bible_summary(school_name: Optional[str] = "낙생고등학교", subject: Optional[str] = None, db: Session = Depends(get_db)):
        ...
        for ans in q.answers:
            if ans.is_accepted or q.is_resolved:
                ...
                recent_archive.append({
                    ...
                    "exam_title": q.exam_type or "내신시험",
                    "question_num": q.question_num,
                    "question_text": q.question_text[:80] + "..." if len(q.question_text) > 80 else q.question_text,
                    "source_book": ans.source_book_name or "기출/시중교재",
                    "source_book_name": ans.source_book_name or "기출/시중교재",
                    "source_detail": ans.source_detail or "",
    ...
    for ti in tracer_items:
        ...
        recent_archive.append({
            ...
            "exam_title": ti.semester or "기출시험",
            "question_num": f"{ti.question_num}번" if not str(ti.question_num).endswith("번") else str(ti.question_num),
            "question_text": ti.question_text or f"{target_school} {ti.subject} 기출 문항",
            "source_book": clean_b,
            "source_book_name": clean_b,
            "source_detail": ti.matched_source or ti.problem_book_source or "",
...
def get_qa_posts(viewer_student_id: Optional[int] = None, db: Session = Depends(get_db)):
    posts = db.query(models.QAPost).filter(models.QAPost.deleted_at == None).order_by(models.QAPost.created_at.desc()).all()
    
    viewer_student = db.query(models.Student).filter(models.Student.id == viewer_student_id).first() if viewer_student_id else None
    viewer_tutor = db.query(models.TutorProfile).filter(models.TutorProfile.student_id == viewer_student_id).first() if viewer_student_id else None
    ...
    for p in posts:
        ...
        # 선배 타깃 핀(Pin) 및 매칭 사유 계산
        p.is_pinned_for_viewer = False
        p.viewer_target_match_reason = None
        
        if viewer_student:
            # 1. 1:1 직접 지목 질문
            if p.target_type == "DIRECT_TUTOR" and ((viewer_tutor and p.target_tutor_id == viewer_tutor.id) or p.target_tutor_id == viewer_student.id):
                p.is_pinned_for_viewer = True
                p.viewer_target_match_reason = "1:1 지목 질문"
            # 2. 모교 선배 타깃 질문
            elif p.target_type == "HIGH_SCHOOL" and p.target_high_school and viewer_student.high_school:
                v_sch = viewer_student.high_school.replace("고등학교", "").replace("고", "").strip()
                t_sch = p.target_high_school.replace("고등학교", "").replace("고", "").strip()
                if v_sch and t_sch and (v_sch in p.target_high_school or t_sch in viewer_student.high_school):
                    p.is_pinned_for_viewer = True
                    p.viewer_target_match_reason = f"모교({viewer_student.high_school}) 후배 질문"
            # 3. 목표 대학 선배 타깃 질문
            elif p.target_type == "TARGET_UNIV" and p.target_university:
                univ_to_match = viewer_tutor.university if (viewer_tutor and viewer_tutor.university) else (viewer_student.target_univ if viewer_student.is_alumni else viewer_student.target_univ)
                if univ_to_match:
                    v_u = univ_to_match.replace("대학교", "").replace("대", "").strip()
                    t_u = p.target_university.replace("대학교", "").replace("대", "").strip()
                    if v_u and t_u and (v_u in p.target_university or t_u in univ_to_match):
                        p.is_pinned_for_viewer = True
                        p.viewer_target_match_reason = f"목표 대학({p.target_university}) 후배 질문"

    # 타깃 매칭된 글 최상단 배치 -> 미해결 글 우선 -> 최신순
    posts.sort(key=lambda x: (
        1 if getattr(x, "is_pinned_for_viewer", False) else 0,
        0 if getattr(x, "is_resolved", False) else 1,
...
def get_parent_weekly_dossier(student_id: int, db: Session = Depends(get_db)):
    ...
    # 2. 주간 순공 시간 및 목표대학 부합도 산출 (실제 DB 기록 100% 연동)
    ...
    # 최근 7일 기록이 없을 경우 전체 누적 세션 확인
    if weekly_hours == 0:
        all_sessions = db.query(models.StudySession).filter(models.StudySession.student_id == student_id).all()
        if all_sessions:
            total_sec = sum(s.duration_sec for s in all_sessions if s.duration_sec)
            weekly_hours = round(total_sec / 3600, 1)
    ...
    if weekly_hours > 0:
        ...
        bench_status = "학습 기록 없음 (타이머 측정 필요)"

    # 3. 출결 & 자습 텔레메트리 (실제 출결 로그)
    att_logs = db.query(models.AttendanceLog).filter(models.AttendanceLog.student_id == student_id).all()
    checkin_count = len(att_logs)
    if checkin_count > 0:
        att_rate = min(100, round((checkin_count / 5.0) * 100))
        att_label = f"{att_rate}% ({checkin_count}회)"
        att_status_desc = "등하원 체크인 정상 기록 중"
    else:
        att_rate = 0
        att_label = "0% (0회)"
        att_status_desc = "이번 주 등원 체크인 기록 없음"

    # 4. 실전 모의고사 및 OMR 취약점 분석 (실제 OMR 응시 데이터)
    ...
    exam_list = []
    for o in omr_subs:
        wrong_cnt = len(o.wrong_questions) if isinstance(o.wrong_questions, list) else (len(json.loads(o.wrong_questions)) if o.wrong_questions and isinstance(o.wrong_questions, str) and o.wrong_questions.startswith('[') else 0)
        exam_list.append({
            "id": o.id,
            "subject": o.subject or "국어",
            "week": getattr(o, "exam_week", 3) or 3,
            "score": o.total_score or 0,
            "grade": o.estimated_grade or 1,
            "wrong_count": wrong_cnt,
            "created_at": o.created_at.strftime("%m/%d %H:%M") if o.created_at else "최근 응시"
        })

    # 5. 과제 수행률 및 기상/취침 루틴 (실제 미션 로그)
    ...
    wake_rate_str = f"{wake_success}회 완료" if wake_success > 0 else "미참여"
    sleep_rate_str = f"{sleep_success}회 완료" if sleep_success > 0 else "미참여"
    ...
    if tier_num == 4:
        ...
        strengths = f"기상({student.wake_target_time or '06:30'}) 및 취침 루틴 관리 중이며, {student.target_univ or '목표대'} 합격선 기준 주간 순공({weekly_hours}시간)을 바탕으로 자기통제 훈련을 진행 중입니다."
        weaknesses = "실패의 원리 핵심 지표 분석 결과, 모의고사 오답 발생 시 단순 해설지 정독으로 넘어가는 미세 회피 습관을 경계해야 합니다. 오답 문항은 본인의 사고 과정을 백지에 직접 복기하여 출제자의 기계적 논리와 대조해야 합니다."
        milestones = f"2027 수능 시험 대비 D-Day 점검, 차주 모의평가 오답 심층 복기 및 {student.target_univ or '목표 대학'} 원서 라인업 점검이 예정되어 있습니다."
        parent_guide = f"어머님, 지금 시기는 무리한 사설 모의고사 추가보다 기출 텍스트 분석과 규칙적인 수면 관리에 집중하는 것이 최선입니다. 아이의 주간 순공({weekly_hours}시간) 페이스를 유지할 수 있도록 격려해 주십시오."
    elif tier_num == 3:
        strengths = f"주간 순공 {weekly_hours}시간 달성으로 {student.target_univ or '목표 대학'} 지원자 표본 기준 견고한 학습량을 유지하고 있습니다."
        weaknesses = f"모의고사 원점수 기준 {student.target_univ or '목표 대학'} 합격선 대비 취약 영역 백분위 보완이 필요합니다."
        milestones = "차주 전국연합학력평가 및 모평 성적 분석, 2027 정시 합격선 모의 배치표 업데이트."
        parent_guide = "안정적인 학습 흐름을 이어갈 수 있도록 가정 내 집중 환경을 조성해 주십시오."
    elif tier_num == 2:
        strengths = "학원 주간 과제 완수율 및 테스트 성적 양호, 정규 커리큘럼 참여 중."
        weaknesses = "주간 테스트 오답 문항에 대한 학원 보충 클리닉 과제 이행 확인 권장."
        milestones = "차주 학원 정규 파이널 실전 모의고사 및 단원별 종합 평가 실시."
        parent_guide = "학원 정규 진도에 맞추어 주간 복습 과제를 기한 내 마칠 수 있도록 확인 부탁드립니다."
    else:
        strengths = f"주간 순공 {weekly_hours}시간 측정 및 기본 학습 진행 중."
    ...
    # 주간 성실도 백분위 실데이터 연산
    diligence = student.diligence_score or 1500
    percentile_str = "상위 2.4%" if diligence >= 2000 else ("상위 5.8%" if diligence >= 1800 else "상위 15.2%")
    ...
    return {
        ...
        "student": {
            ...
            "grade": student.grade or 3,
            "target_univ": student.target_univ or "목표 대학 미설정",
            "target_dept": getattr(student, 'target_dept', '') or "",
            "baseline_univ": student.baseline_univ or "마지노선 대학 미설정",
            "baseline_dept": getattr(student, 'baseline_dept', '') or "",
            "academy_code": student.academy_code or "일반"
        },
        "tuition_cycle": {
            "total_weeks": 4,
        ...
        "attendance_telemetry": {
            ...
            "attendance_rate": att_label,
            "late_count": 0,
            "absence_count": 0,
            "status_label": att_status_desc
        },
        "exams": exam_list,
        "homework_and_habits": {
            "homework_rate": "정규 과제 점검 완료",
